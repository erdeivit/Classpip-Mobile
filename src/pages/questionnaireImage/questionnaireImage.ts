...
@Component({
  selector: 'page-questionnaireImage',
  templateUrl: './questionnaireImage.html'
})
  ...
  public doSubmitAnswer(event) {
    ...
    if((this.indexNum) < this.numTotalQuestions){
      ...
      switch (this.questions[this.indexNum].type) {
          case 'textArea':
            this.navController.setRoot(QuestionnaireTextAreaPage, { myQuestionnaire: this.myQuestionnaire, myCredentials: this.myCredentials, questions: this.questions, indexNum: this.indexNum, numAnswerCorrect: this.numAnswerCorrect, numAnswerNoCorrect: this.numAnswerNoCorrect, dataAnswers: this.dataAnswers });
            break;
          case 'image':
            this.navController.setRoot(QuestionnaireImage1Page, { myQuestionnaire: this.myQuestionnaire, myCredentials: this.myCredentials, questions: this.questions, indexNum: this.indexNum, numAnswerCorrect: this.numAnswerCorrect, numAnswerNoCorrect: this.numAnswerNoCorrect, dataAnswers: this.dataAnswers });
            break;
          default:
            break;
        }
    ...
    /*this.questionnaireService.saveResults(this.utilsService.currentStudent, this.myQuestionnaire, this.myQuestionnaire.name, this.myQuestionnaire.id, this.numTotalQuestions, this.numAnswerCorrect, this.numAnswerNoCorrect, this.finalNote, this.dataAnswers).subscribe(
      ((value: ResultQuestionnaire) => this.navController.setRoot(CompletedQuestionnaire1Page, { results: value, myQuestionnaire: this.myQuestionnaire, myQuestions: this.questions, myCredentials: this.myCredentials })),
      error =>
        this.ionicService.showAlert(this.translateService.instant('APP.ERROR'), error));
      }*/

      this.questionnaireService.getMyStudent(this.utilsService.currentUser.userId).subscribe(
      ((value: Student) => this.navController.setRoot(CompletedQuestionnaire1Page, { student: value, myQuestionnaire: this.myQuestionnaire, numTotalQuestions: this.numTotalQuestions, numAnswerCorrect: this.numAnswerCorrect, numAnswerNoCorrect: this.numAnswerNoCorrect, finalNote: this.finalNote, dataAnswers: this.dataAnswers, myQuestions: this.questions, myCredentials: this.myCredentials })),
      error =>
        this.ionicService.showAlert(this.translateService.instant('APP.ERROR'), error));
      }
