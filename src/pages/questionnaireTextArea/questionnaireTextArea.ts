...
@Component({
  selector: 'page-questionnaireTextArea',
  templateUrl: './questionnaireTextArea.html'
})
  ...
  public doSubmitAnswer(event) {
    ...
    if((this.indexNum) < this.numTotalQuestions){
        ...
        switch (this.questions[this.indexNum].type) {
          case 'textArea':
            this.navController.setRoot(QuestionnaireTextArea1Page, { myQuestionnaire: this.myQuestionnaire, myCredentials: this.myCredentials, questions: this.questions, indexNum: this.indexNum, numAnswerCorrect: this.numAnswerCorrect, numAnswerNoCorrect: this.numAnswerNoCorrect, dataAnswers: this.dataAnswers });
            break;
          case 'image':
